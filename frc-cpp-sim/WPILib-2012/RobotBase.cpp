 ...
 */
void RobotBase::robotTask(FACTORYPTR factory, Task *task)
{
 ...
 */
void RobotBase::startRobotTask(FACTORYPTR factory)
{
#ifdef SVN_REV
    if (strlen(SVN_REV))
    {
    ...
    if (oldId != ERROR)
    {
        ...
        // goodness knows what happened... so abort.
	return;
#else
...
#ifdef __FRC_CPP_SIM__
    task->Start((taskarg_t)factory, (taskarg_t)task);
#else
