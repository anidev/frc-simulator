...
#define PRINTF if(TrackAPI_debugFlag)printf
...
{ 
    ...
    PRINTF("%s%s%i%i: particle %i is in (%i %i) height %i width %i\n",LOG_INFO, 
...
int FindColor(FrcHue color, ParticleAnalysisReport* trackReport)
{
    ...
    if ( !success )    { 
        PRINTF ("%s%s%i%i: did not find color - errorCode= %i",LOG_INFO,GetLastVisionError());    
        return success;
    }
        ...
        ColorReport *colorReport, Rect rect)
{
    ...
    if (!success){
        PRINTF("%s%s%i%i: No camera Image available Error = %i %s",LOG_INFO, 
    ...
    if ( !success )    { 
        errorCode = GetLastVisionError(); 
        PRINTF ("%s%s%i%i: Error = %i  %s ",LOG_DEBUG, errorCode, GetVisionErrorText(errorCode));
        frcDispose(__FUNCTION__,cameraImage,histImage,NULL); 
        return success; 
    }    
    ...
    if ( !success )    {
        errorCode = GetLastVisionError(); 
        PRINTF ("%s%s%i%i: Error after GetLargestParticle = %i  %s ",LOG_DEBUG, errorCode, GetVisionErrorText(errorCode));
        frcDispose(__FUNCTION__,cameraImage,histImage,NULL); 
        imaqSetError(ERR_COLOR_NOT_FOUND, __FUNCTION__);
        return success; 
    }
    PRINTF("%s%s%i%i: largestParticleIndex = %i\n",LOG_INFO, largestParticleIndex);
    ...
    /* Particles were found  */
        ...
        if (colorReport != NULL)
        {
            ...
            success = frcParticleFilter(cameraImage, cameraImage, &criteria, 1, options, 
                    rect, &numParticles);
            if ( !success )    { 
                PRINTF("%s%s%i%i: frcParticleFilter errorCode %i",LOG_INFO, GetLastVisionError()); 
            }    
            ...
            ColorHistogramReport* chrep = imaqColorHistogram2(histImage, numClasses, IMAQ_HSL, 
                        NULL, cameraImage);
            if (chrep == NULL) { 
                PRINTF("%s%s%i%i: NULL Color Histogram",LOG_INFO);
                errorCode = GetLastVisionError(); 
            } else {
...
{
    printf("%s%s%i%i: particle analysis:\n    %s%i  %s%i\n    %s%lf\n    %s%i  %s%i\n    %s%g  %s%g\n    %s%g\n    %s%i  %s%i\n    %s%i  %s%i\n",LOG_INFO,
    ...
    printf("%s%s%i%i: quality statistics:  \n    %s%g %s%g \n",LOG_INFO,
...
{
    printf("%s%s%i%i: particle ranges for %s%i particles:     ",LOG_INFO,
...
{
    printf("%s%s%i%i: name of color: %s",LOG_INFO, myReport->name);
