...
#include <cstdlib>
#include <cstring>
#include <fioLib.h>        // included even in simulator for fioRead
#include <unistd.h>
...
#define PRINTF if(AxisCamera_debugFlag)printf

/** @brief Camera data to be accessed globally */
struct _globalCamera {
...
int CameraInitialized()
{
    ...
    if (!globalCamera.cameraReady)  {
        ...
        PRINTF ("%s%s%i%i: Camera request before camera is initialized",LOG_DEBUG);
    ...
    if (globalCamera.index == -1){
        ...
        PRINTF ("%s%s%i%i: No camera image available",LOG_DEBUG);
...
int GetImage(Image* image, double *timestamp)
{
    ...
    while (readCount) {
        ...
        if (!imaqDuplicate(image, globalCamera.data[readIndex].decodedImage)) {
            ...
            PRINTF ("%s%s%i%i: Error duplicating image= %i  %s ",LOG_DEBUG, errorCode, GetVisionErrorText(errorCode));            
        }
    ...
    if (currentTime > globalCamera.data[globalCamera.index].timestamp + CAMERA_IMAGE_STALE_TIME_SEC){
        PRINTF ("%s%s%i%i: STALE camera image (THIS COULD BE A BAD IMAGE)",LOG_CRITICAL);
...
int GetImageData(char** imageData, int* numBytes, double* currentImageTimestamp)
{
    ...
    while (readCount) {
        ...
        if (NULL == cameraImageString) {
                    PRINTF ("%s%s%i%i: Unable to allocate cameraImage",LOG_DEBUG);
                    globalCamera.cameraMetrics[CAM_GETIMAGE_FAILURE]++;
                    return success;
        }
...
{
    PRINTF ("%s%s%i%i: Closing socket - CAMERA ERROR: %s",LOG_CRITICAL, errstring );
    close (socket);
    return (ERROR);
}
...
static int CameraOpenSocketAndIssueAuthorizedRequest(const char* serverName, const char* request) 
{
    ...
    for (;i < numAuthenticationStrings;++i) {
        ...
        //PRINTF ("%s%s%i%i: creating camSock",LOG_DEBUG ); 
        ...
        //PRINTF ("%s%s%i%i: connecting camSock",LOG_INFO ); 
        ...
        //PRINTF ("%s%s%i%i: writing GET request to camSock",LOG_DEBUG ); 
...
int ConfigureCamera(char *configString){
    ...
    PRINTF("%s%s%i%i: camera configuration string: \n%s",LOG_DEBUG, cameraRequest);
    camSock = CameraOpenSocketAndIssueAuthorizedRequest(serverName, cameraRequest);
    PRINTF("%s%s%i%i: camera socket# = %i",LOG_DEBUG, camSock);
    
    //read response
    success = CameraSkipUntilEmptyLine(camSock);
    //PRINTF("%s%s%i%i: succcess from CameraSkipUntilEmptyLine: %i",LOG_DEBUG, success);
    char buffer[3];    // set property - 3
    success = CameraReadLine(camSock, buffer, 3, true);
    //PRINTF("%s%s%i%i: succcess from CameraReadLine: %i",LOG_DEBUG, success);
    PRINTF("%s%s%i%i: line read from camera \n%s",LOG_DEBUG, buffer);
    if (strcmp(buffer, "OK") != 0) {
        imaqSetError(ERR_CAMERA_COMMAND_FAILURE, funcName);
        PRINTF("%s%s%i%i: setting ERR_CAMERA_COMMAND_FAILURE - OK not found",LOG_DEBUG);
    }
...
int GetCameraSetting(char *configString, char *cameraResponse){
    ...
    PRINTF("%s%s%i%i: camera configuration string: \n%s",LOG_DEBUG, cameraRequest);
    camSock = CameraOpenSocketAndIssueAuthorizedRequest(serverName, cameraRequest);
    PRINTF("%s%s%i%i: return from CameraOpenSocketAndIssueAuthorizedRequest %i",LOG_DEBUG, camSock);
    
    //read response
    success = CameraSkipUntilEmptyLine(camSock);
    success = CameraReadLine(camSock, cameraResponse, 1024, true);
    PRINTF("%s%s%i%i: succcess from CameraReadLine: %i",LOG_DEBUG, success);
    PRINTF("%s%s%i%i: line read from camera \n%s",LOG_DEBUG, cameraResponse);
...
int cameraJPEGServer(int frames, int compression, ImageResolution resolution, ImageRotation rotation)
{
    ...
    PRINTF ("%s%s%i%i: cameraJPEGServer",LOG_DEBUG ); 
    ...
    switch (resolution) {
        ...
        default: {PRINTF ("%s%s%i%i: code error - resolution input",LOG_DEBUG ); break; }
    }
...
Authorization: Basic %s;\n\n";
    ...
    PRINTF("%s%s%i%i: revised camera string: \n%s",LOG_DEBUG, getStr);
    /* Allocation */
    ...
    while (1)
    {
      ...
      PRINTF ("%s%s%i%i: getting IP",LOG_DEBUG );
      ...
      PRINTF ("%s%s%i%i: Attempting to connect to camSock",LOG_INFO ); 
      if (connect (camSock, (struct sockaddr *) &cameraAddr, sockAddrSize) == ERROR)     {
        imaqSetError(ERR_CAMERA_CONNECT_FAILED, funcName);
    ...
    //PRINTF ("%s%s%i%i: reading header",LOG_DEBUG ); 
...
#define MEASURE_TIME 0
    ...
    while (cont) {
        ...
        //PRINTF ("%s%s%i%i: Completed fioRead function - bytes read:%d",LOG_DEBUG, bytesRead);
        if (bytesRead <= 0) {
        ...
        if (globalCamera.decode) {
            ...
            if (! Priv_ReadJPEGString_C(globalCamera.data[writeIndex].decodedImage, 
                    ...
                    globalCamera.data[writeIndex].cameraImageSize) ) {
                PRINTF ("%s%s%i%i: failure creating Image",LOG_DEBUG);            
            }
        }
...
#ifndef __FRC_CPP_SIM__ // FIXME not supported on linux
	/* signal a listening task */
        if (globalCamera.readerPID) {
            if (taskKill (globalCamera.readerPID,SIGUSR1) == OK) {
                PRINTF ("%s%s%i%i: SIGNALING PID= %i",LOG_DEBUG, globalCamera.readerPID);
	    }
            else {
                globalCamera.cameraMetrics[CAM_PID_SIGNAL_ERR]++;
                PRINTF ("%s%s%i%i: ERROR SIGNALING PID= %i",LOG_DEBUG, globalCamera.readerPID);
	    }
        }
#endif
        ...
        if (printCounter == 1000) { 
            PRINTF ("%s%s%i%i: imageCounter = %i",LOG_DEBUG, globalCamera.cameraMetrics[CAM_NUM_IMAGE]); 
            printCounter=0; 
        }
        ...
        if (performanceLoopCounter <= maxCount) {
            PRINTF ("%s%s%i%i: %i DONE!!!: loop = ,%g,  camera = ,%g,  difference = ,%g, loopRate= ,%g,",LOG_DEBUG,
...
{    
    ...
    PRINTF("%s%s%i%i: starting acquisition",LOG_DEBUG);
}
...
static int initCamera(int frames, int compression, ImageResolution resolution, ImageRotation rotation) 
{
    ...
    PRINTF("%s%s%i%i: \n+++++ camera task starting: rotation = %i",LOG_DEBUG, (int)rotation);
    int errorCode;
    ...
    PRINTF ("%s%s%i%i: calling cameraJPEGServer",LOG_DEBUG ); 
    errorCode = cameraJPEGServer(frames, compression, resolution, rotation);    
    PRINTF ("%s%s%i%i: errorCode from cameraJPEGServer = %i\n",LOG_INFO, errorCode ); 
    return (OK);
}
...
int StartCameraTask(int frames, int compression, ImageResolution resolution, ImageRotation rotation)
{
    ...
    PRINTF("%s%s%i%i: starting camera",LOG_DEBUG);
    ...
    PRINTF("%s%s%i%i: spawned task id %i",LOG_DEBUG, cameraTaskID);

    if (!started)    {
        PRINTF("%s%s%i%i: camera task failed to start",LOG_DEBUG);
