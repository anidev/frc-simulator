...
std::unique_ptr<Entry> Reader::ReadEntry(bool useLastValue)
{
    switch (Check(useLastValue))
    {
    case kNetworkTables_BOOLEAN_FALSE:
        return std::unique_ptr<Entry>(new BooleanEntry(false));
    case kNetworkTables_BOOLEAN_TRUE:
        return std::unique_ptr<Entry>(new BooleanEntry(true));
    case kNetworkTables_INT:
        return std::unique_ptr<Entry>(new IntegerEntry(ReadInt()));
    case kNetworkTables_DOUBLE:
        return std::unique_ptr<Entry>(new DoubleEntry(ReadDouble()));
    case kNetworkTables_STRING:
        return std::unique_ptr<Entry>(new StringEntry(ReadString().c_str()));
    default:
