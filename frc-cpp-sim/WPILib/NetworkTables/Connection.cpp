...
void Connection::OfferTransaction(NetworkQueue *transaction)
{
    ...
    for (; !transaction->IsQueueEnd(it); it++)
    {
        // Re-offer as an unique_ptr if it was before
        if (it->second)
            m_queue->Offer(std::unique_ptr<Data>(it->first));
        else
...
void Connection::Offer(std::unique_ptr<Data> autoData)
{
    Synchronized sync(m_dataLock);
    ...
    m_queue->Offer(std::move(autoData));
    semGive(m_dataAvailable);
}
...
void Connection::Start()
{
    ...
    m_watchdogTask.Start((taskarg_t)this);
    m_readTask.Start((taskarg_t)this);
    m_writeTask.Start((taskarg_t)this);
#else
    ...
    while (m_connected)
    {
        ...
        if (value >= kNetworkTables_ID || value == kNetworkTables_OLD_DATA)
        {
            ...
            if (ConnectionManager::GetInstance()->IsServer() && ConfirmationsContainsKey(key))
            {
                if (m_inTransaction)
                    ...
                    Offer(std::unique_ptr<Data>(new Denial(1)));
            ...
            else if (value >= kNetworkTables_TABLE_ID)
            {
                ...
                else
                {
                    ...
                    if (m_inTransaction)
                    {
                        m_transaction->Offer(std::unique_ptr<Data>(tableEntry));
                    }
                    else
                    {
                        key->GetTable()->Got(false, key, std::unique_ptr<Entry>(tableEntry));
                        Offer(std::unique_ptr<Data>(new Confirmation(1)));
                    }
                }
            }
            else
            {
                std::unique_ptr<Entry> entry = input.ReadEntry(true);
                if (!m_connected)
                    break;
                ...
                else
                {
                    ...
                    if (m_inTransaction)
                    {
                        m_transaction->Offer(std::unique_ptr<Data>(entry.release()));
                    }
                    else
                    {
                        key->GetTable()->Got(false, key, std::move(entry));
                        Offer(std::unique_ptr<Data>(new Confirmation(1)));
                    }
                }
            }
        ...
        else if (value >= kNetworkTables_CONFIRMATION)
        {
            ...
            while (count-- > 0)
            {
                ...
                {
                    entry->GetKey()->GetTable()->Got(true, entry->GetKey(), std::unique_ptr<Entry>(entry));
                }
            }
        }
        ...
        else if (value == kNetworkTables_TABLE_REQUEST)
        {
            ...
            {
                Synchronized sync(m_dataLock);
                Offer(std::unique_ptr<Data>(new TableAssignment(table, id)));
                table->AddConnection(this);
            }
...
{
    std::unique_ptr<Buffer> buffer = std::unique_ptr<Buffer>(new Buffer(2048));
    bool sentData = true;
    while (m_connected)
    {
        ...
        buffer->Flush(m_socket);
        // Noone else wants this data and it used to be unique_ptr'd, so delete it
        if (data.second)
            delete data.first;
    }
}
